...
alias -l moor return moo: $iif($mooi(name),$moof(os) $moof(up) $moof(cpu) $moof(gfx) $moof(res) $moof(ram) $moof(hdd) $moof(net) $moof(sound) $moof(mobo),lookup error)
...
alias -l moof {
  ...
  if ($1 == sound) { return $moos($1) $mooi(sound) }
  if ($1 == mobo) { return $moos($1) $mooi(mobo) }
}
